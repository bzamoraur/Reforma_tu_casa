...
test('completes Level 3 end-to-end, shows the scorecard, and persists progress', async ({
  page,
}) => {
  const errors: string[] = [];
  ...
  // The menu offers Level 3 directly; start it.
  const start = page.locator('[data-testid="start-level"][data-level-id="level-3"]');
  ...
  await page.reload();
  await expect(
    page.locator('[data-testid="progress-badge"][data-level-id="level-3"]'),
  ).toBeVisible();
